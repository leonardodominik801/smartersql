...
A smarter interface to SQL.

:copyright: The watchdog project. http://watchdog.net/
...
class Table(object, metaclass=metatracker):
    ...
    def _createSQL(cls):
        ...
        for k, v in cls.columns.items():
            ...
            if v.notnull: 
                x += ' NOT NULL'
            if v.default: 
                x += ' DEFAULT %s' % x.default
            
            x += ',\n'
        
        if cls.primary:
            x += '  PRIMARY KEY (%s)\n' % ', '.join(v.sql_name for v in cls.primary.values())
        else:
            x = x[:-2] + '\n' # remove last comma
    ...
    @classmethod
    def create(cls): 
        cls.db.query(cls._createSQL())
    
    @classmethod
    def drop(cls, cascade=False): 
        cls.db.query(cls._dropSQL(cascade))
...
class Column(object):
    ...
    toxml = lambda self, obj: '>' + web.htmlquote(str(obj))
